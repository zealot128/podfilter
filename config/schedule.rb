...
every 10.minutes do
  command "pgrep -f -u stefan 'sidekiq #{Sidekiq::VERSION} prod' || (rm -f /apps/podfilter/prod/current/log/sidekiq.log && cd /apps/podfilter/prod/current && ~/.rvm/bin/rvm 2.1.1 do bundle exec sidekiq -d -i 0 -P /apps/podfilter/prod/current/tmp/pids/sidekiq.pid -e production -L /apps/podfilter/prod/current/log/sidekiq.log)"
end
